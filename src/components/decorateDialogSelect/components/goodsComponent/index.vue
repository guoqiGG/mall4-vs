<template>
  <div class="goods-modal-content">
    <div class="goods-form">
      <el-form @submit.native.prevent inline ref="searchForm" :model="searchForm" label-width="108px">
  ...
  methods: {
    ...
    handleCancelSelected (rows, row) {
      ...
      if (this.isMulilt) { // 如果当前是多选
        ...
        if (!selected) {
          ...
          this.multipleSelection = this.removeDuplicCateObj([...this.multipleSelection, row])
          if (this.goodsNumber != 0) { // 如果不为0的情况下，说明用户限制了当前选择的数量
            if (this.multipleSelection.length > this.goodsNumber) {
              let delRow = this.multipleSelection.shift()
      ...
      if (this.dataUrl) { // 活动url存在
        ...
        this.$http({
          ...
          params: this.$http.adornParams({
            current: page,
            size: perPage,
            shopCategoryId: this.searchForm.categoryId, // 商品分类
    ...
    setDataShow (val) {
      ...
      this.$nextTick(() => {
        ...
        const prodIds = this.multipleSelection.map(x => x.prodId)
        this.tableList.forEach(item => {
          if (prodIds.indexOf(item.prodId) > -1) {
            this.$refs.multipTable.toggleRowSelection(item, true)
          }
        })
