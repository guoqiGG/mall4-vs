<template>
  <div style="position: relative;">
    <div
      ...
      :style="{height: (parseInt(setSize.imgHeight)) + 'px'}"
    >
      <div
        ...
        >
          ...
          <div
            ...
            :style="{'width':Math.floor(parseInt(setSize.imgWidth)*47/310)+ 'px',
                     'height': setSize.imgHeight,
                     'top':'-' + (parseInt(setSize.imgHeight) + (setSize.parentWidth * 0.04)) + 'px',
...
export default {
  ...
  data () {
    return {
      ...
      // 移动中样式
      moveBlockBackgroundColor: '#155BD4',
  ...
  methods: {
    init () {
      ...
      setTimeout(() => {
        this.$nextTick(() => {
          const setSize = that.resetSize(that)// 重新设置宽度高度
          for (const key in setSize) {
            that.$set(that.setSize, key, setSize[key])
          }
          that.$parent.$emit('ready', that)
        })
      })
    ...
    start: function (e) {
      ...
      if (this.isEnd == false) {
        this.text = ''
        this.moveBlockBackgroundColor = '#155BD4'
      ...
      if (this.status && this.isEnd == false) {
        ...
        reqCheck(data).then(res => {
          ...
          if (res.repCode == '0000') {
            ...
            this.passFlag = true
            this.tipWords = `${((this.endMovetime - this.startMoveTime) / 1000).toFixed(2)}s${this.$i18n.t('verifySuccessfully')}`
            ...
            if (res.repCode === '6110') {
              this.tipWords = this.$i18n.t('verificationCodeHasExpired')
            } else {
              this.tipWords = this.$i18n.t('verificationFailure')
            }
    ...
    getPictrue () {
      const data = {
        captchaType: this.captchaType,
        clientUid: localStorage.getItem('bbcSlider'),
        ts: Date.now() // 现在的时间戳
      }
