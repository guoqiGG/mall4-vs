  ...
  <el-dialog title="违规管理"
    ...
    <el-form @submit.native.prevent :model="dataForm"
      ...
      <el-form-item label="处理状态"
        ...
        <div v-if="dataForm.status === 1">平台下线</div>
        <div v-else-if="dataForm.status === 2">等待审核</div>
        <div v-else-if="dataForm.status === 3">审核通过</div>
        <div v-else-if="dataForm.status === 4">审核不通过</div>
        <div v-else>其他</div>
    ...
    <span slot="footer"
          class="dialog-footer">
      <div @click="visible = false" class="default-btn">取消</div>
      <div class="default-btn primary-btn"
                 :disabled="dataForm.status !== 1 && dataForm.status !== 4"
                 @click="dataFormSubmit()">提交申请</div>
    </span>
  </el-dialog>
</template>
