...
const mainRoutes = {
  ...
  beforeEnter (to, from, next) {
    let authorization = Vue.cookie.get('bbcAuthorization_vs')
...
router.beforeEach(async (to, from, next) => {
  ...
  if (router.options.isAddDynamicMenuRoutes || fnCurrentRouteType(to, globalRoutes) === 'global') {
    ...
    http({
      ...
      sessionStorage.setItem('bbcMenuList', JSON.stringify(data.menuList || '[]'))
...
router.onError((error) => {
  ...
  const isChunkLoadFailed = error.message.match(jsPattern) || error.message.match(cssPattern)
...
function fnAddDynamicMenuRoutes (menuList = [], routes = []) {
  ...
  if (temp.length >= 1) {
    ...
    sessionStorage.setItem('bbcDynamicMenuRoutes', JSON.stringify(mainRoutes.children || '[]'))
  }
}
