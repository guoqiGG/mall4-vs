  ...
  >
    ...
    >
      ...
      <el-form-item :label="$t('transport.chargePrice')" prop="chargeType">
        <template>
          <el-radio-group v-model="dataForm.chargeType" :disabled="dataForm.isFreeFee == 1" @change="changeChargeType">
...
export default {
  ...
  methods: {
    ...
    getTransport () {
      if (this.dataForm.transportId) {
        this.$http({
          ...
          this.dataForm = data
          if (!this.dataForm.transfeeFrees.length) {
            this.dataForm.transfeeFrees = [{ freeCityList: [] }]
          }
    ...
    changeChargeType (type) {
      this.dataForm.transfees.forEach(item => {
        item.firstPiece && this.checkChargeType(item, 'firstPiece', type)
        item.continuousPiece && this.checkChargeType(item, 'continuousPiece', type)
      })
      this.dataForm.transfeeFrees.forEach(item => {
        item.piece && this.checkChargeType(item, 'piece', type)
      })
    },
    // 价格格式化
    convertPrice (row, attr) {
      const price = +this.checkAmount(row[attr])
      row[attr] = price.toFixed(2)
    },
    // 根据收费方式格式化输入的值
    checkChargeType (row, attr, chargeType) {
      // 0：按件数  1：按重量  2：按体积
      if (chargeType === 0) {
        const number = this.getNumber(row[attr])
    ...
    formWriteHint () {
      this.$message({
        message: this.$i18n.t('shop.pleaseWriteInfo'),
        type: 'error',
        duration: 1500
      })
    },
