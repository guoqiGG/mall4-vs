  ...
  >
    ...
    >
      ...
      <el-form-item :label="this.$i18n.t('station.businessStatus')">
        <el-radio-group v-model="status" :disabled="dataForm.status > 1">
  ...
  export default {
    data () {
      ...
      var validateStationTime = (rule, value, callback) => {
        ...
        this.timeDateModeVO.stationStartTime = parseInt(this.timeDateModeVO.stationStartTime) || 0
        ...
        } else if (this.timeDateModeVO.stationStartTime > this.timeDateModeVO.stationEndTime) {
          callback(new Error(this.$i18n.t('admin.pickUpTimeError')))
        } else {
          callback()
        }
      }
    ...
    created () {
      console.log('station-add-or-update')
    },
    methods: {
      startChange (val) {
        if (!val) {
      ...
      selectProvince (val) {
        ...
        // this.cityList = this.getCurrentChild(this.provinceList, val)
        const {curNode, areas} = this.getCurrentChild(this.provinceList, val)
        if (areas) {
          this.cityList = areas
        } else {
      ...
      selectCity (val) {
        ...
        const {curNode, areas} = this.getCurrentChild(this.cityList, val)
        if (areas) {
          this.areaList = areas
        } else {
          this.listAreaByParentId(val).then(({ data }) => {
      ...
      compareTime (starTimestamp, endTimestamp) {
        let starTime = new Date(starTimestamp)
        let starHours = starTime.getHours()
        let starMinute = starTime.getMinutes()
        let starSecond = starTime.getSeconds()
        ...
        if (starHours > endHours) {
          return true
        } else if (starHours === endHours) {
          if (starMinute > endMinute) {
            return true
          } else if (starMinute === endMinute) {
            if (starSecond > endSecond) {
              return true
            } else if (starSecond === endSecond) {
              return true
            } else {
              return false
            }
      ...
      // }
        ...
        if (timeDateMode.shopTime) {
          ...
          if (this.compareTime(timeDateMode.shopTime[0], timeDateMode.shopTime[1])) {
        ...
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            this.isSubmit = true
            this.dataForm.status = this.dataForm.status > 1 ? this.dataForm.status : this.status
            this.$http({
