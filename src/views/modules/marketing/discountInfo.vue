...
export default {
  data () {
    var validateTime = (rule, value, callback) => {
      ...
      if (this.dataForm.status === 1 && rule.field === 'endTime' && new Date() > Date.parse(endTime)) {
  ...
  mounted () {
    ...
    //   sessionStorage.setItem('bbcDiscountInfo-params', JSON.stringify(this.$route.query || '{}'))
    // }
    // let sessionStorageParams = JSON.parse(sessionStorage.getItem('bbcDiscountInfo-params'))
    if (this.$route.query.discountId !== undefined) {
