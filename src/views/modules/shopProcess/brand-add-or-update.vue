              ...
              >
                ...
                >
                  <template slot-scope="scope">
                    <ImgShow :src="scope.row.imgUrl" :imgStyle="{width:'57px',height:'57px'}" />
                  </template>
                </el-table-column>
                <el-table-column
                  prop="qualifications"
...
export default {
  ...
  methods: {
    ...
    imgChange (value) {
      if (value) {
        this.$refs.bankCartInfoForm.validate().catch(() => { })
      }
    },
    signBrandImgChange (value) {
      if (value) {
        this.$refs.signBrandForm.validate().catch(() => { })
      }
    },
    ...
    getPlatformBrand () {
      this.$http({
        ...
        const signBrandList = this.signBrandForm.signBrandList
    ...
    getSignBrandList () {
      this.$http({
        ...
        this.signBrandForm.signBrandList = data.platformBrandList
    ...
    addBrand (brandObj, index) {
      const length = this.signBrandForm.signBrandList.length + this.dataForm.customSignBrandList.length + this.signingCount
      if (length >= 50) {
        this.$message.warning(this.$i18n.t('shopProcess.brandMaxLimitTips'))
      } else {
        if (brandObj.isSelected === false) {
          this.signBrandForm.signBrandList.push(brandObj)
        }
     ...
     */
    addCustomBrand () {
      const length = this.signBrandForm.signBrandList.length + this.dataForm.customSignBrandList.length
      if (length >= 50) {
        this.$message.warning(this.$i18n.t('shopProcess.brandMaxLimitTips'))
        return
      }
    ...
    deleteBrand (id, index) {
      ...
      this.signBrandForm.signBrandList.splice(index, 1)
    },
    ...
    signBrand () {
      ...
      this.signBrandForm.signBrandList.forEach(item => {
        const param = {
     ...
     * 确认提交
     */
    comfirmSubmit: Debounce(function () {
      let pass = true
      if (this.signBrandForm.signBrandList.length) {
        this.$refs['signBrandForm'].validate((valid) => {
          if (!valid) {
            pass = false
          }
        })
      ...
      /* if (this.signBrandForm.signBrandList.find(el => !el.qualifications)) {
        this.$message.error(this.$i18n.t('shopProcess.brandQualificationsNotEmpty'))
        pass = false
        return
      } */
      // if (!this.signBrandForm.signBrandList.length && !this.dataForm.customSignBrandList.length) {
