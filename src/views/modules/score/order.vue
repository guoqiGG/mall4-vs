<template>
      ...
      <el-form @submit.native.prevent :inline="true" :model="dataForm" @keyup.enter.native="getDataList(this.page)"
        ...
        <div class="input-row">
          ...
          <el-form-item :label="$t('order.contactTel') + ':'">
            <el-input style="min-width: 350px;" v-model="dataForm.mobile"
              :placeholder="this.$i18n.t('order.pleaseEnterNumber')" clearable size="small" type="number"
          ...
          size="small">导出待发货订单</el-button>-->
            <div class="default-btn" @click="clear()">{{ $t("order.clear") }}</div>
            <div class="default-btn" @click="getSoldExcel()">{{ $t("order.export") }}</div>

            <div class="default-btn" @click="uploadSpu">{{ $t("order.BulkShipping") }}</div>
          </el-form-item>
        </div>
      </el-form>
    </div>
                  ...
                  <div class="items name" v-for="orderItem in order.orderItems" :key="orderItem.orderItemId">
                    ...
                    <div class="prod-name">
                      <dir class="prod-con">
                        ...
                        <div class="order-status" v-if="order.status > 0 && order.status < 8">
                          {{
                            [
                              "",
  ...
  : ""
                    ...
                    <span class="totalprice" v-if="order.freightAmount">（{{ $t("order.includingFreight") }}：{{
              ...
              <el-col :span="3" style="height: 100%">
                <div class="item">
                  <div>
                    ...
                    <!-- <span
                      v-else
                    >{{['氢春豆支付','微信小程序支付','支付宝PC端支付','微信扫码支付','微信H5支付','微信公众号支付','支付宝H5支付','支付宝APP支付','微信APP支付','用户余额支付','PayPal支付'][order.payType]}}</span>-->
                  </div>
                </div>
              </el-col>
              <el-col :span="3" style="height: 100%">
              ...
              <el-col :span="2" style="height: 100%">
                <div class="item">
                  ...
                  <span>
                    ...
                    <span v-else-if="order.status === 3" size="small"
                      type="danger">{{ $t("order.pendingReceipt") }}</span>
                    <span v-else-if="order.status === 7" size="small" type="danger">{{ $t("group.waitGroup") }}</span>
                    <span v-else-if="order.status === 5" size="small"
                      type="danger">{{ $t("order.successfulTransaction") }}</span>
              ...
              <el-col :span="3" style="height: 100%">
                <div class="item">
                  <div class="operate">
                    ...
                    <div class="default-btn text-btn operate-btn" @click="addOrUpdateHandle(order.orderNumber)">
                      {{ $t("order.seeDetails") }}</div>
                    <div class="default-btn text-btn operate-btn marLeftNo block" @click="toImbox(order)">
                      {{ $t("order.contactBuyer") }}</div>
                    <div class="default-btn text-btn operate-btn marLeftNo" @click="changeOrder(order)" v-if="order.status === 2 &&
                    ...
                    <div class="default-btn text-btn marLeftNo" v-if="order.status == 3 &&
                      (!order.refundStatus || order.refundStatus > 2) &&
                      order.dvyType === 1
                      " @click="reviseLogistics(order.orderNumber)">{{ $t("delivery.modifyLogistics") }}</div>
                  </div>
                </div>
              </el-col>
            </el-row>
    ...
    <el-dialog :title="this.$i18n.t('delivery.modifyLogistics')" :close-on-click-modal="false" :visible.sync="logVisible"
      @closed="handleClose" class="LogModification" width="50%">
      <!-- 修改 -->
          ...
          <div class="log-list">
            <div class="item" v-for="(logItem, index) in logisticsInfo" :key="index">
              ...
              <el-form @submit.native.prevent :model="logDataForm" ref="logDataForm"
                @keyup.enter.native="logDataFormSubmit()" label-width="95px" size="small">
                <el-form-item :label="$t('delivery.deliveryMethod') + '：'" style="text-align: left">
                ...
                <div class="info-int" v-if="logItem.deliveryType !== 3">
                  <el-form-item :label="$t('delivery.courierCompany') + '：'" class="form-item">
                  ...
                  <el-form-item :label="$t('delivery.trackingNumber') + '：'" class="form-item">
                    <el-input v-model="logItem.dvyFlowId" controls-position="right" :min="0"
                      :class="['int-frame', errorNum == 1 ? 'errortip' : '']"></el-input>
      ...
      <div class="change-logistics" v-if="!isReviseLog">
        <div class="warning">{{ $t("delivery.ifModifyTheLog") }}</div>
        <el-table header-cell-class-name="table-header" row-class-name="table-row" :data="confirmList">
          <!-- <div v-for="(item,index) in confirmList" :key="index"> -->
          <el-table-column property="dvyIdName" :label="this.$i18n.t('delivery.packageName')" width="220"
            style="text-align: left"></el-table-column>
          <el-table-column property="productNums" :label="this.$i18n.t('delivery.amountOfGoods')"
            width="140"></el-table-column>
          <el-table-column property="delMethod" :label="this.$i18n.t('delivery.deliveryMethod')"
            width="140"></el-table-column>
          <el-table-column property="dvyName" :label="this.$i18n.t('delivery.logisticsCompany')"
            width="170"></el-table-column>
          <el-table-column property="dvyFlowId" :label="this.$i18n.t('delivery.trackingNumber')"></el-table-column>
          <!-- </div> -->
        </el-table>
      </div>
      ...
      <div slot="footer" v-if="!isReviseLog" class="dialog-footer">
        ...
        <div class="default-btn primary-btn" type="primary" @click="confirmRevise()">{{
