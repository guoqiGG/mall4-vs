<template>
  ...
  >
    ...
    <el-upload
      class="upload-demo"
      ref="upload"
      :action="$http.adornUrl('/user/user/importCustomerExcel')"
...
<script>
  export default {
    ...
    methods: {
      ...
      downloadModel () {
        this.$http({
          url: this.$http.adornUrl('/user/user/downloadTemplate'),
          method: 'get',
          params: this.$http.adornParams({templateType: this.type}),
          responseType: 'blob' // 解决文件下载乱码问题
        }).then(({ data }) => {
          var blob = new Blob([data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=utf-8' })
          const fileName = this.type === 1 ? this.$i18n.t('user.importMemberMode') : this.$i18n.t('user.importClientMode')
