<template>
  ...
  <div>
    ...
    >
      ...
      <!-- /分页 -->
              <el-alert
                :title="this.$i18n.t('user.sendCouponTips')"
          type="warning"
          show-icon
          :closable="false">
        </el-alert>
      <span slot="footer">
        <div @click="visible = false" class="default-btn">{{$t('user.cancel')}}</div>
        <div class="default-btn primary-btn" @click="submitProds()">{{$t('user.confirm')}}</div>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getDataList (page) {
      ...
      this.$http({
        url: this.$http.adornUrl('/admin/coupon/listCanSendCoupon'),
        method: 'get',
        ...
        this.dataList.forEach(item => {
          // 平台投放 / 用户领取
          item.couponType = item.couponType === 1 ? this.$t('product.voucher') : (item.couponType === 2 ? this.$t('coupon.discountVoucher') : (item.couponType === 3 ? this.$t('coupon.coinCertificate') : ''))
          // item.eachObtain = 1
    ...
    confirm: Debounce(function () {
      ...
      console.log(param)
      this.$http({
        url: this.$http.adornUrl(`/admin/coupon/sendUserCoupon`),
        method: 'put',
        data: this.$http.adornData(param)
      }).then(({ data }) => {
        this.active = []
        this.$message({
          message: this.$i18n.t('user.success'),
          type: 'success',
          duration: 1500,
          onClose: () => {
            this.visible = false
