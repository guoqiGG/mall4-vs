  ...
  <div>
    ...
    >
      <el-form :model="dataForm" :rules="dataRule" ref="dataForm" class="demo-form-inline form">
        ...
        <el-form-item v-if="dataForm.status === 0" :label="$t('user.disableRemark')+ ':'" size="mini" prop="disableRemark">
          <el-input v-model="dataForm.disableRemark" maxlength="100" type="textarea" :rows="3"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <div class="default-btn" @click="visible = false">{{$t('user.cancel')}}</div>
        <div class="default-btn primary-btn" @click="confirm">{{$t('user.confirm')}}</div>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  data () {
    return {
      ...
      dataRule: {
        disableRemark: [
          { required: true, message: this.$i18n.t('user.disableRemarkNoNull'), trigger: 'blur' }
        ]
      },
      page: {
        total: 0, // 总页数
