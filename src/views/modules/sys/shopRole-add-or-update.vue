  ...
  methods: {
    init (id) {
      ...
      this.$http({
        ...
        if (this.dataForm.id) {
          nodeList = this.getNode(this.menuList)
        }
      }).then(() => {
        this.visible = true
        this.$nextTick(() => {
        ...
        if (this.dataForm.id) {
          this.$http({
            ...
            let selectMenuIdList = data.menuIdList.filter(x => !nodeList.includes(x))
            this.$refs.menuListTree.setCheckedKeys(selectMenuIdList)
          })
        }
      })
    },
    ...
    getNode (list) {
      ...
      list.forEach(item => {
        if(item.children) {
          res.push(item.menuId, ...this.getNode(item.children))
        }
      })
      return res
    },
