    ...
    >
      ...
      <el-form-item :label="this.$i18n.t('shop.shopAdress') + '：'">
        <span>
          ...
          {{ shopInfo.shopAddress }}（{{ $t("shop.phone") }}：{{shopInfo.tel}})
        </span>
      </el-form-item>
    ...
    async getDataInfo () {
      ...
      this.$http({
        ...
        if (!this.dataForm.samecityId || !this.polygonPath || this.polygonPath.length <= 0) {
          ...
          this.$nextTick(() => {
            this.isConfigureAareaDefault = true
          })
        }
      })
    },
 ...
 */
      ...
      return new Promise((resolve, reject) => {
        this.$http({
          ...
          if (data) {
            ...
            this.dataForm = data
            this.polygonPath = this.dataForm.polygonPath || this.polygonPath
          // this.center = {
    ...
    updatePolygonPath (e) {
      ...
      this.isConfigureAareaDefault = false
    },
    // 鼠标进入多边形
    polygonMouseover () {
      const cls = document.getElementsByClassName('BMap_vectex_node')
      cls[0].parentElement.style['zIndex'] = 700
    },
    // 鼠标离开多边形
    polygonMouseoutr () {
      const cls = document.getElementsByClassName('BMap_vectex_node')
      cls[0].parentElement.style['zIndex'] = 201
    },
    // 表单提交
    resetMapPoint () {
      this.isConfigureAareaDefault = true
 ...
 */
    ...
    dataFormSubmit () {
      ...
      if (this.isConfigureAareaDefault && this.dataForm.status == 0) this.dataForm.polygonPath = []
    ...
    checkData () {
      ...
      // 请设置可配送区域
      if (this.dataForm.status == 1 && this.isConfigureAareaDefault) {
        this.errorMsg(this.$i18n.t('shop.setArea'))
        this.isDataTrue = false
        return
      }
