...
export default {
  ...
  methods: {
    ...
    getMiniPageList () {
      ...
      this.$http({
        url: this.$http.adornUrl('/shop/shopRenovation/pagePC'),
        methods: 'get',
        params: this.$http.adornParams({
          current: pageNum, // 当前页
          size: pageSize, // 每页显示多少条
          renovationType: 1, // 1表示pc端，2表示移动端
          name: this.searchForm.pageName
        })
      }).then(({ data }) => {
    ...
    goCreatePage (id) {
      ...
      let newPage = this.$router.resolve({
        path: '/shop-decorate/create/edit/index',
        query: obj
      })
    ...
    handleDelete (item) {
      this.$confirm(this.$i18n.t('shopFeature.list.deleteTips'), this.$i18n.t('shopFeature.list.tips'), {
        ...
        this.$http({
          url: this.$http.adornUrl('/shop/shopRenovation/deletePC/' + item.renovationId),
          method: 'delete'
        }).then(({ data }) => {
    ...
    handleSetHomePage (renovationId) {
      const that = this
      this.$http({
        url: this.$http.adornUrl('/shop/shopRenovation/updateHomePagePC/' + renovationId),
        method: 'put'
      }).then(({ data }) => {
    ...
    searchChange () {
      this.perProps.pageNum = 1
      this.perProps.pageSize = 10
      this.getMiniPageList()
    }
  }
}
