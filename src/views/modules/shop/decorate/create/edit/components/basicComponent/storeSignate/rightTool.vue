<template>
    ...
    <div class="store-signate-Config">
        <!-- <div class="header-title">{{$t('pcdecorate.storeSignate.businessSigns')}}
            <i class="el-icon-close" @click="closeStoreSignateConfig"></i>
        </div> -->
        <!-- <div class="title-border"></div> -->
        <div class="config-items">
            <div class="title">{{$t('pcdecorate.shopMessage.storeMessage')}}</div>
            <div class="right-select">
                <el-radio-group v-model="configForm.storeStyle" @change="handleStoreStyle">
                    <el-radio :label="0">{{$t(`pcdecorate.storeSignate.style1`)}}</el-radio>
                    <el-radio :label="1">{{$t(`pcdecorate.storeSignate.style2`)}}</el-radio>
                </el-radio-group>
            </div>
        </div>
        <div class="config-items">
            ...
            <div class="right-select">
                <el-radio-group v-model="configForm.showNavs" class="special-radio">
        ...
        <div class="config-items" v-show="configForm.showNavs === 0">
            ...
            <div class="right-select">
                <el-radio-group v-model="configForm.showCategory" class="special-radio">
                    <el-radio :label="0">{{$t(`pcdecorate.storeSignate.show`)}}</el-radio>
                    <el-radio :label="1">{{$t(`pcdecorate.storeSignate.hide`)}}</el-radio>
                </el-radio-group>
            </div>
        ...
        <div class="config-items">
            <div class="title" style="padding-top: 9px">{{$t(`pcdecorate.storeSignate.navsBack`)}}</div>
            <div class="right-select" style="display: flex;align-items: center">
                <el-color-picker
                ...
                <span @click="handleBgReset" style="margin: 0 15px;color: #155BD4;white-space: nowrap;cursor: pointer;">{{$t('pcdecorate.axinterval.reset')}}</span>
        ...
        <div class="config-items nav-solid" v-show="configForm.showNavs === 0">
            <div class="navs-config">
                <div class="title">{{$t('shopFeature.searchBar.search')}}</div>
                <div class="right-select">
            ...
            <div class="navs-config">
                <div class="title">{{$t('shopFeature.allCanUse.notice')}}</div>
                <div class="right-select">
            ...
            <template v-if="configForm.navList && configForm.navList.length > 0">
                <div class="navs-items" 
                    ...
                    <el-row class="navs-hovers" style="margin: 10px 5px;position: relative;">
                        <el-col :span="11" style="display: flex;font-size: 12px;align-items: center;">
        ...
        <div class="config-items config-bg" v-show="configForm.storeStyle != 0">
  ...
  methods: {
    ...
    refreshPic (imagePath) {
      this.configForm.bgUrl = this.resourcesUrl + imagePath
        ...
        // 删除招牌背景图
    handleRemmoveImage () {
      this.configForm.bgUrl = ''
    },
        // 导航背景重置
    handleBgReset () {
      this.configForm.navsBg = 'rgba(0, 0, 0, 1)'
    },
        // 店铺样式触发
    handleStoreStyle () {
      if (this.configForm.storeStyle == 0) {
        this.configForm.bgUrl = ''
      }
    },
        // 验证信息
    handleValidate () {
      ...
      if (JSON.stringify(this.editItem) === ' {}') {
        status = false
        message = this.$i18n.t('pcdecorate.storeSignate.warning3')
      } else if (this.editItem.showNavs === 0) {
