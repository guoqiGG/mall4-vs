            ...
            <div class="hot-preivew-container">
                <draggable 
                    ...
                    <div 
                        ...
                        :key="index"
                        class="ad-image-hot">
                        <div class="add-ad-image" @click="showHotAreaPop(index)">{{$t(`pcdecorate.univerHot.addHot`)}}</div>
                        <div class="add-ad-image" @click="changeImg(index)">{{$t(`pcdecorate.univerHot.updateImage`)}}</div>
  ...
  data () {
    return {
      ...
      boxItem: {// 盒子模板
        link: '',
        type: '',
        title: this.$t('pcdecorate.univerHot.jumpLinks'),
        left: 0,
        top: 0,
        translateX: 0,
        translateY: 0,
  ...
  methods: {
    ...
    handleDialogSubmit ({type, value}) {
      if (type === '1') { // 当前选择的是商品
        ...
        this.cacheImageActiveBox[this.currentHotEditIndex].title = value.customLink.url // 自定义链接名称
