          ...
          <div id="scrollbar" ref="designEditor" :style="{'overflow': 'auto', 'background': pageBackgroundColor}" class="design-container">
            <div style="width:100vw">
              <div class="design-editor" :style="scaleFun">
                <!-- <div 
                  class="components-management" 
                  @click="handleClickComponent">{{$t('pcdecorate.commonModal.component')}}</div> -->
                  <!-- 商家招牌 start -->
            ...
            <!-- 招牌头部配置 start -->
                  ...
                  <div v-show="isRightShow" class="right-toolbars" :class="isRightShow && 'fade-open'">
                    <right-config-message
                      ...
                      :currentUseComponents="currentUseComponents.concat(HeaderComponent)"
  ...
  methods: {
    ...
    view () {
      ...
      let newPage = this.$router.resolve({
        path: '/shop-decorate/create/select-decorate/index',
        query: {
          type: 'detail'
        }
      })
      ...
      for (let i = 0; i < currentArr.length; i++) {
        for (let j = 0; j < backReturnArr.length; j++) {
          if (currentArr[i][type] == backReturnArr[j].prodId) {
            ...
            currentArr[i].price = currentArr[i].price == null ? backReturnArr[j].price : ''
    ...
    addComponent (item) {
      ...
      if (item.type === 'shop_signs' && isUse != -1) {
        return this.$message.warning('商家招牌只能添加一个！')
      }
      if (item.type === 'shop_signs') {
        ...
        for (let i = 0; i < newList.length; i++) {
          if (newList[i].type === 'shop_signs') {
            if (newList[0].type != 'shop_signs') {
              this.$message.warning('商家招牌组件只能显示在头部！')
            }
          }
        }
      }
      ...
      setTimeout(() => {
        this.currentUseComponents.forEach((item, index) => {
          if (this.currentIndex == index) {
            ...
            // this.storeHeaderStatus = false // 头部配置信息隐藏
            this.configMessage = {
    ...
    sortStart (e) {
      ...
      // document.getElementsByClassName('draggerStyles')[0].style.top = (e.node.offsetTop) + 'px'
    },
    // 当拖拽时鼠标移动时触发
    sortMove (event) {
    ...
    handleSave (type) {
      ...
      if (isValidateArr.length > 0 || isValidateHeaderArr.length > 0) { // 判断验证情况
        ...
        if (this.renovationId) {
          url = this.$http.adornUrl('/shop/shopRenovation/updatePC')
          method = 'put'
          tips = this.$t('shopFeature.edit.saveSuccess')
        } else {
          url = this.$http.adornUrl('/shop/shopRenovation/savePC')
          method = 'post'
          tips = this.$t('shopFeature.edit.saveSuccess')
        }
        if (type === '1') {
    ...
    handleSaveTemplate (dataForm) {
      ...
      if (isValidateArr.length > 0 || isValidateHeaderArr.length > 0) { // 判断验证情况
        ...
        let tips = ''
        if (this.templateId) {
          url = this.$http.adornUrl('/shop/shopTemplate/updatePC')
          method = 'put'
          tips = this.$t('shopFeature.edit.saveSuccess')
        } else {
          url = this.$http.adornUrl('/shop/shopTemplate/savePC')
          method = 'post'
          tips = this.$t('shopFeature.edit.saveSuccess')
        }
        // if (type === '1') {
    ...
    handleUpdateMessage (obj) {
      ...
      // 头部信息
      this.HeaderComponent.forEach(v => {
        if (v.Ref + '-1' === obj.ref) {
