export default {
  ...
  methods: {
    ...
    getMiniPageList () {
      ...
      this.$http({
        url: this.$http.adornUrl('/shop/shopTemplate/pagePC'),
        methods: 'get',
    ...
    copyTemplatePage (templateId) {
      this.$http({
        url: this.$http.adornUrl(`/shop/shopTemplate/copyPC/${templateId}`),
        method: 'post'
      }).then(({ data }) => {
    ...
    handleDelete (renovationId) {
      this.$confirm(this.$i18n.t('shopFeature.list.deleteTips'), this.$i18n.t('shopFeature.list.tips'), {
        ...
        this.$http({
          url: this.$http.adornUrl('/shop/shopTemplate/deletePC/' + renovationId),
          method: 'delete'
        }).then(({ data }) => {
