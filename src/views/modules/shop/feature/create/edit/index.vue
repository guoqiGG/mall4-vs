        ...
        <div class="design-page-content">
          ...
          <div id="scrollbar" class="design-editor" :class="currentOperationType === 'detail' ? 'view-class' : ''">
            <div ref="designBox" class="design-box">
              <div ref="designPreview" class="design-preview" :style="{margin: currentOperationType!='detail' ? '30px 350px 0 0' : '0'}">
                          ...
                          :key="item.type">
                            <div class="preview-item">
                              <div
                                ...
                                :currentUseComponents="goodsWaterfallComponent"
                                :index-key="item.type+'_'+index"
                                :data-field="item.dataField || {}"
                                :current-component="item"
                                :is-check-my-self="item.isCheckMySelf"
                                :is-start-check-field-rules="isStartCheckFieldRules"
                                @topFixed="topFixed"
                                @componentsValueChance="goodsWaterfallComponentValueChance"
                                @save="save"></div>
                              <!--边框、删除、添加-->
                              <div
                                v-if="currentOperationType!='detail'"
            ...
            <div class="design-config-editor">
              ...
              <div class="components-list-edit">
                <p class="clear-current-component" :style="[{color:setOther.viewTheme}]">
                  ...
                  <span
                    @click="currentUseComponents = []; goodsWaterfallComponent = []"
                ...
                <template v-if="goodsWaterfallComponent.length > 0">
                  <div
                    ...
                    <div class="drag-box-icon">
                      <i :title="$i18n.t('shopFeature.edit.removeComponent')" class="el-icon-delete" @click="popBtn(1, item,'pop_'+index,index)"/>
                    </div>
                  </div>
                </template>
                <!-- 商品瀑布 end -->
...
export default {
  ...
  data () {
    return {
      ...
      pageType: '',
      savePageType: '0', // 保存类型
      goodsWaterfallComponent: [], // 底部瀑布组件
      goodsWEdit: false // 是否修改瀑布组件
    }
  },
  watch: {
    microId (val) {
  ...
  mounted () {
    ...
    //   this.viewContent(JSON.parse(localStorage.getItem('bbcViewContent')))
    } else {
      this.configComponents = JSON.parse(JSON.stringify(this.currentConfigComponents))
    }
    if (this.pageType === 'detail' && renovationId) {
      this.getInfo()
    } else if (this.pageType === 'detail' && localStorage.getItem('bbcViewContent')) {
      this.viewContent(JSON.parse(localStorage.getItem('bbcViewContent')))
    }
  ...
  methods: {
    ...
    toView () {
      const $data = this.configComponents.concat(this.currentUseComponents, this.goodsWaterfallComponent)
      const par = {
     ...
     *  */
    addComponent (item) {
      if (item.type === 'goodsWaterfall' && this.goodsWaterfallComponent.length === 1) {
        return this.$message.warning(this.$i18n.t('shopFeature.goodsWaterfall.tip2'))
      }
      let goodsWaterfallComponent = this.goodsWaterfallComponent
      ...
      if (item.type != 'goodsWaterfall') {
        ...
        this.goodsWEdit = false
      } else {
        this.currentEditComponent = -1
        this.goodsWEdit = true
        goodsWaterfallComponent.push($par)
      }
      setTimeout(() => {
        // 重置插入位置为空
        this.$forceUpdate()
        this.currentUseComponents = allCurrentUseComponents
        this.goodsWaterfallComponent = goodsWaterfallComponent // 瀑布组件
    ...
    // 点击商品瀑布组件
    clickgoodsWaterfallComponent ($item) {
      this.currentEditComponent = -1
      this.goodsWEdit = true
      this.reSetIsShowEdit()
      this.componentOperation()
    ...
    popBtn ($type, item, ref, index) {
      ...
      // this.$refs[ref][0].showPopper = falses
      this.isShowEditorAllComponents = false
    },
    // 监听某个组件的变化
    ...
    goodsWaterfallComponentValueChance ($val) {
      if ($val) {
        this.goodsWaterfallComponent[$val.current].dataField = $val.dataField
      }
    },
    // 拖拽
    sortStart () {
        ...
        for (let i = 0; i < val.length; i++) {
          if (val[i].type === 'goodsWaterfall') {
            if (val[val.length - 1].type != 'goodsWaterfall') {
              this.$message.warning(this.$i18n.t('shopFeature.goodsWaterfall.tip1'))
            }
          }
        }
      // 始终保证招牌在顶部
        let arr = []
        let goodsWaterfallComponent = []
        val.forEach(item => {
          if (item.type === 'goodsWaterfall') {
            goodsWaterfallComponent.push(item)
          } else {
            arr.push(item)
          }
        })
    ...
    // 验证完毕之后发布push
    savePush () {
      const $parameter = this.configComponents.concat(this.currentUseComponents, this.goodsWaterfallComponent)
    ...
    async saveData ($data, saveType) {
      ...
      let method = ''
      if (this.renovationId) {
        url = this.$http.adornUrl('/shop/shopRenovation/updateMove')
        method = 'put'
      } else {
        url = this.$http.adornUrl('/shop/shopRenovation/saveMove')
        method = 'post'
      }
      this.$http({
        url: url,
    ...
    async saveTemplateData ($data, saveType) {
      ...
      let method = ''
      if (this.templateId) {
        url = this.$http.adornUrl('/shop/shopTemplate/updateMove')
        method = 'put'
      } else {
        url = this.$http.adornUrl('/shop/shopTemplate/saveMove')
        method = 'post'
      }
      this.$http({
        url: url,
    ...
    getInfo () {
      this.$http({
        url: this.$http.adornUrl('/shop/shopRenovation/info/' + this.renovationId),
        method: 'GET'
      }).then(async({ data }) => {
        this.configComponents = []
        let par = {}
        ...
        if ($data.length) {
          ...
          $data.map(re => {
            ...
            if (re.type.indexOf('config') !== -1) {
              ...
              this.goodsWaterfallComponent.push(re)
            } else {
              this.currentUseComponents.push(re)
            }
          })
    ...
    getTemplateDetail () {
      this.$http({
        url: this.$http.adornUrl('/shop/shopTemplate/info/' + this.templateId),
        method: 'GET'
      }).then(async({ data }) => {
        this.configComponents = []
        if (this.pageType === 'edit') {
    ...
    async matchproducts (arr) {
      ...
      arr.forEach(item => {
        if (item.type === 'goods') { // 商品列表
          ...
          console.log(item.dataField.leftConfig.goodsList)
