      ...
      <div class="table-con prod-table">
        <div ref="closepopover" class="table-con prod-table">
          <el-table ref="prodListTable" :data="dataList" header-cell-class-name="table-header" row-class-name="table-row"
            ...
            <el-table-column align="left" prop="totalStocks" :label="$t('product.availableInventory')" width="140"
              sortable>
              <!-- <template slot-scope="scope">
  ...
  methods: {
    ...
    batchChangeProdStatus(status) {
      ...
      if (FailList.length > 0) {
        FailList.forEach(el => {
          console.log(el)
          this.$message.warning(this.$i18n.t('home.product') + '（' + el.prodName + '）' + this.$i18n.t('product.cannotBeRemovedOrRemovedFromTheChassis'))
        })
        return
      }
