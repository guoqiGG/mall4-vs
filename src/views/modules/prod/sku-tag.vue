                  ...
                  >
                    <div class="item">
                      <div class="prop-value">
                        <div class="text-hid">{{ tagItem.propValue}}</div>
                        <!-- 第一个规格允许上传图片 -->
...
export default {
    ...
    this.$http({
      ...
      if (data) {
        ...
        this.$http({
          ...
          //   const fd = item.prodPropLangList.find(fdItem => fdItem.lang === this.masterLangInfo.lang)
          //   item.propName = fd ? fd.propName : item.prodPropLangList[0].propName
          // })

          if (this.skuList.length) {
            // this.skuList.forEach(sku => {
            //   const fdSkuLang = sku.skuLangList && sku.skuLangList.find(f => f.lang === this.masterLangInfo.lang)
            //   if (fdSkuLang) {
            //     sku.prodName = fdSkuLang.prodName
            //     sku.properties = fdSkuLang.properties
            //     sku.skuName = fdSkuLang.skuName
            //   }
            // })
            console.log('重新init的this.skuListthis.skuListthis.skuList', this.skuList)
            this.$emit('resetInit', this.skuList)
  ...
  computed: {
    ...
    unUseTags () {
      ...
      console.log('未使用的规格', res)
      return res
    },
    skuTags: {
      get () {
        console.log('当前所选的sku值', this.$store.state.prod.skuTags)
      ...
      handler (val, oldVal) {
        ...
        if (this.type === 4) {
          ...
          this.skuList.forEach((sku, idx) => {
            ...
            if (propertiesArray[this.tagNameIndex] && this.tagItemName !== propertiesArray[this.tagNameIndex].substring(propertiesArray[this.tagNameIndex].indexOf(':') + 1)) {
              skuList.push(sku)
            }
          })
        } else if (this.type === 2) {
  ...
  methods: {
    init (skuList) {
      this.value = skuList
      console.log('初始化的skuList', skuList)
      ...
      for (let i = 0; i < skuList.length; i++) {
        ...
        for (let j in propertiesArray) {
          let cnProperties = [propertiesArray[j].slice(0, propertiesArray[j].indexOf(':')), propertiesArray[j].substring(propertiesArray[j].indexOf(':') + 1)]
          // console.log(propertiesArray[j], cnProperties)
          if (!skuTags[j]) {
            skuTags[j] = {
    ...
    changeTagName (selVal, index) {
      ...
      // this.$i18n.t('language') === 'English' && oldSkuTag.tagNameEn !== '' ? oldSkuTag.tagNameEn :
      let oldValue = oldSkuTag.tagName
      if (selVal !== oldValue) {
        // 规格发生变化
        this.type = 3
      }
    ...
    changeTagItemPic (value, skuTagItem, tagIndex, tagItemIndex) {
      ...
      if (skuTag && skuTagItem) {
        ...
        for (let i = 0; i < this.skuList.length; i++) {
          ...
          let pName = (this.skuList[i].properties.split(';'))[0]
          let properties = pName.substring(pName.indexOf(':') + 1)
    ...
    getTagValues (key, usedItems, str) {
      console.log('getTagValues', key, usedItems)
      const fd = this.dbTags.find(fdItem => fdItem.propName === key)
      if (fd) {
        const result = fd.prodPropValues.map(item => {
        // 默认显示主语言的信息
          const master = item.prodPropValueLangList.find(m => m.lang === this.masterLangInfo.lang)
