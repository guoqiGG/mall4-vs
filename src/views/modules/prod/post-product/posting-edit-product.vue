  ...
  methods: {
      ...
      this.$http({
        ...
        if (data) {
          ...
          if (!this.dataForm.prodLangList.length) {
            ...
            this.dataForm.prodParameterList.forEach(item => {
              let masterParInx = 0
              item.prodParameterLangList.forEach((parame, parInx) => {
                if (parInx !== 0 && parame.lang === info.lang) {
                  masterParInx = parInx
                }
                const fd = this.langItemList.find(f => f.lang === parame.lang)
                if (fd) {
    ...
    getCategoryInfo () {
      ...
      this.$http({
        ...
        this.currentShopCategoryObj = {
          firstCat: {
            dataList: data,
            id: this.dataForm.shopCategoryId
          }
        }
      })
    },
    ...
    selectLang (value) {
      ...
      // 参数设置
      this.dataForm.prodParameterList.forEach(item => {
        const flList = item.prodParameterLangList.filter(fl => this.curLang.includes(fl.lang))
        this.curLang.forEach((langItem, index) => {
    ...
    skuTagChangeSkuHandler (skuList, skuTags, type) {
      ...
      // const prodNameCn = this.dataForm.prodLangList[0] ? this.dataForm.prodLangList[0].prodName : ''
      let stocks = 0
      skuList.forEach(sku => {
