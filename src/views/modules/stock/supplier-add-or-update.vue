<template>
  <el-dialog
    ...
    <el-form @submit.native.prevent :inline="true" :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" :label-width="labelWidth">
    ...
    data () {
      ...
      var validMobile = (rule, value, callback) => {
        if (value == null || value === '') {
          ...
          callback(new Error(this.$i18n.t('language') === '简体中文' ? '请输入正确的手机号' : 'Please enter valid mobile'))
        } else {
          callback()
        }
      }
      ...
      var validPhone = (rule, value, callback) => {
        if (value == null || value === '') {
          ...
          callback(new Error(this.$i18n.t('language') === '简体中文' ? '请输入正确的座机号' : 'Please enter valid phone number'))
        } else {
          callback()
        }
      }
      var validateQq = (rule, value, callback) => {
        if (value == null || value === '') {
          ...
          callback(new Error(this.$i18n.t('language') === '简体中文' ? '请输入正确的QQ号' : 'Please enter valid qq'))
        } else {
          callback()
        }
      }
      ...
      var validateWx = (rule, value, callback) => {
        if (value == null || value === '') {
          ...
          callback(new Error(this.$i18n.t('language') === '简体中文' ? '请输入正确的微信号' : 'Please enter valid wx'))
        } else {
          callback()
        }
      }
      return {
        ...
        dialogWidth: '',
        defWidth: 815,
        labelWidth: localStorage.getItem('bbcLang') === 'en' ? '130px' : '100px'
      }
    },
    created () {
