...
export default {
  ...
  methods: {
      ...
      if (this.dataForm.stockBillLogId) {
        this.$http({
          ...
          data.stockBillLogItems.forEach(item => {
            item.totalAmount = new Big(item.stockCount).times(item.unitPrice).valueOf()
          })
    ...
    changStock (row) {
      ...
      this.$set(row, 'purchaseAmount', new Big(stockCount).times(row.unitPrice)).valueOf()
      this.setTotalAmountAndCount()
    },
    changPrice (row) {
      ...
      this.$set(row, 'purchaseAmount', new Big(row.stockCount).times(unitPrice)).valueOf()
      this.setTotalAmountAndCount()
    },
    setTotalAmountAndCount () {
      ...
      this.prods.forEach(item => {
        ...
        totalCount = new Big(totalCount).plus(item.stockCount).valueOf()
      })
            ...
            if (valid) {
              ...
              this.$http({
                ...
                this.$message({
                  ...
                  duration: 1500,
                  onClose: () => {
                    this.$router.push('/stock-purchasesInbound')
                    this.isSubmit = false
                  }
                })
              }).catch((e) => {
    ...
    supplierProdSelectHandle (data) {
      data.forEach(prod => {
        if (this.containsId(prod)) {
          ...
          this.$set(prod, 'purchaseAmount', new Big(prod.purchasePrice).times(prod.minOrderQuantity).valueOf())
    ...
    refreshDataList (data) {
      ...
      if (data.data) {
        ...
        list.forEach(prod => {
          ...
          this.$set(prod, 'unitPrice', prod.purchasePrice)
          this.$set(prod, 'price', prod.purchasePrice)
          this.$set(prod, 'purchaseAmount', new Big(prod.purchasePrice).times(prod.stockCount || prod.minOrderQuantity).valueOf())
