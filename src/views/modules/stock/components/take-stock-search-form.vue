      ...
      <el-form @submit.native.prevent :inline="true" class="search-form" ref="searchForm" :model="searchForm" label-width="auto" size="small">
        ...
        <div class="input-row">
          <el-form-item :label="$t('takeStock.createTime')+':'">
            ...
            <div class="default-btn" style="margin-left: 20px;" :class="{ 'is-active': createTimeActive === 1 }" @click="setDateRange(1)">{{
              $t("time.t")
            }}</div>
            <div class="default-btn" :class="{ 'is-active': createTimeActive === 2 }" @click="setDateRange(2)">{{
              $t("time.y")
            }}</div>
            <div class="default-btn" :class="{ 'is-active': createTimeActive === 3 }" @click="setDateRange(3)">{{
              $t("time.n")
            }}</div>
            <div class="default-btn" :class="{ 'is-active': createTimeActive === 4 }" @click="setDateRange(4)">{{
              $t("temp.m")
            }}</div>
          </el-form-item>
          <el-form-item>
