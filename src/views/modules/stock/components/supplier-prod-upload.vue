<template>
  ...
  <el-dialog
    ...
    <div class="tips">
      ...
      <p>{{$t('shop.exportProdTip2')}}</p>
      <p>{{$t('shop.exportProdTip3')}}</p>
      <p>{{$t('shop.exportProdTip4')}}</p>
    </div>
    <div style="display: inline-block">
      <el-upload
        class="upload-demo"
        ref="upload"
        :action="$http.adornUrl('/supplier/supplierProd/importExcel')"
        ...
        <div v-if="isAuth('multishop:supplierProd:import')" class="default-btn download-btn" @click="submitUpload">
          {{ $t("product.import") }}</div>
        <div v-if="isAuth('multishop:supplierProd:downLoad')" class="default-btn download-btn" @click="downloadModel">
...
export default {
  ...
  methods: {
    ...
    downloadModel () {
      this.$http({
        url: this.$http.adornUrl('/supplier/supplierProd/downLoadModel'),
        method: 'get',
        responseType: 'blob' // 解决文件下载乱码问题
      }).then(({ data }) => {
        var blob = new Blob([data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=utf-8' })
        const fileName = this.$i18n.t('shop.prodFileName')
