<template>
  <div class="mod-stock-receive-list">
    <!-- 顶部按钮 -->
    <div v-if="isAuth('multishop:receiveStock:add')" class="default-btn primary-btn" @click="addOrUpdateHandle(null, null)">{{ this.$i18n.t('stock.newlyBuildOtherStorage') }}</div>
    <div style="height: 10px"></div>
    <!-- 搜索栏组件 -->
    <stockBillSearchFrom :type="2" :search-form="searchForm" @searchChange="searchChange(true)" @exportStockLog="exportStockLog"/>
    <div style="height: 10px"></div>
    <!-- 状态导航选择组件 -->
    <statusSelectTabs :type="2" :statusArr="statusArr" :defaultKey="statusArr[0].key" @statusChange="statusChange"/>
    <!-- 表格组件 -->
    <stockBillTable ref="stockBillTable" class="stockBillTable" :type="2" :dataList="dataList" @refreshList="getDataList"/>
    <!-- 分页 -->
    <el-pagination
  ...
  data () {
    return {
      ...
      statusArr: [
        ...
        { id: 2, key: 1, label: this.$i18n.t('stock.inStorage') },
  ...
  methods: {
    ...
    confirmExport () {
      ...
      this.$http({
        ...
        const fileName = this.$i18n.t('stock.stockBillLogXls')
