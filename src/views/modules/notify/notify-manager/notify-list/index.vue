<template>
  <div class="Mall4j mod-prod-list">
    ...
    <div class="search-bar">
      <el-form @submit.native.prevent :inline="true" :model="searchParam" size="small" label-width="180">
        <div class="input-row">
          <!-- 商品名称 -->
          ...
          >
            <template slot-scope="{ row }">
              <!-- <div class="table-btn-con"> -->
                <el-button type="text" @click="toReleasePage(row)">{{ $t('crud.viewBtn') }}</el-button>
              <!-- </div> -->
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  methods: {
    ...
    // 批量已读
    batchRead (ids) {
      let type = ids.length > 0 ? 0 : 1
      this.$http({
        url: this.$http.adornUrl('/multishop/notifyLog/is_read?type=' + type),
        method: 'put',
        data: ids
      }).then(({ data }) => {
        this.$message({
