<template>
  <div class="distribution-recruit-set gray-box top-redius border-bottom-gray">
    <div class="title">{{ $t("user.pointsQuestionSetting") }}</div>
    <el-form @submit.native.prevent
      ...
      :model="dataForm"
    >
      <el-form-item :label="$t('user.pointsQuestionSetting')" prop="paramValue">
        <tiny-mce
          ref="tinyMce"
        ...
        if (valid) {
          this.$http({
            ...
            data: this.$http.adornData({
              ...
              'remark': this.$i18n.t('user.pointsQuestionSetting')
            })
          }).then(({ data }) => {
            // this.getData()
            this.$message({
