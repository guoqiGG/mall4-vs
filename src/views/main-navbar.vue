  ...
  methods: {
    ...
    openWs () {
      ...
      this.imSocketTask = new WebSocket(
        ...
        `/im/websocket/online/shop/${Base64.encode(this.$cookie.get('bbcAuthorization_vs'))}/${this.shopId}/${this.employeeId}`)
      )
      ...
      this.imSocketTask.onmessage = (res) => {
        ...
        if (result.code === '00000' && result.data) {
          // localStorage.getItem('bbcImBox')防止已在在消息盒子页面右下方弹出不停弹出
          this.message = 1
          if (result.data.newMessage && this.notificationDebounce && !localStorage.getItem('bbcImBoxShop')) {
    ...
    showMessage: Debounce(function () {
      this.$message({
        message: this.$i18n.t('publics.newUnreadMessages'),
        offset: 10,
        duration: 1500
      })
    }, 1800),
