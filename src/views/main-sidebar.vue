  ...
  <aside class="site-sidebar">
    <div class="menu-mod">
      ...
      <div class="menu-left">
        ...
        <ul>
          <li v-for="menu in menuList" v-if="!menu.hidden && menu.name != showName" class="menu-left-active">
            <div :class="{'menu-left-active': selectLeftId === menu.menuId, 'menu-left-item1' : $i18n.t('language') === 'English','menu-left-item' : $i18n.t('language') !== 'English'}" v-if="menu.list" @click="expandMenu(menu)">
              ...
              ></icon-svg>
              <span class="item-text" :title="menu.name&&menu.name.length > 4 ? menu.name : ''" style="font-size: 14px">{{ menu.name }}</span>
            </div>
...
<script>
  ...
  export default {
    ...
    created () {
      this.menuList = JSON.parse(sessionStorage.getItem('bbcMenuList') || '[]')
      this.dynamicMenuRoutes = JSON.parse(sessionStorage.getItem('bbcDynamicMenuRoutes') || '[]')
    ...
    methods: {
      ...
      toHome () {
        ...
        this.expandMenuList = []
        sessionStorage.setItem('bbcIsExpand', '0')
      ...
      expandMenu (menu, flag) {
        ...
        if (menu.list) {
          sessionStorage.setItem('bbcIsExpand', '1')
        }
        if (id1 !== id2) {
          this.routeJump(menu)
        }
