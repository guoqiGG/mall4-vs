        ...
        <div class="chat-box-left">
          ...
          >
            ...
            >
              ...
              <img :src="item.pic" class="shop-icon"  @error="item.pic=''" />
              </div>
                  ...
                  <div class="row" v-for="(row, index) in msgList" :key="index">
                    ...
                    <div class="my" v-if="row.source == 2">
                      ...
                      <div class="left">
                        ...
                        />
                        <img :src="row.userHeadUrl" class="avatar" v-else @error="row.userHeadUrl=''" />
                      </div>
                      <!-- 左-头像 -->
                      <div class="right">
                        <div class="username">
                    ...
                    >
                      ...
                      <div class="left">
                        <div class="username">
                          <div class="name">{{ row.nickName || row.shopName }}</div>
                        </div>
...
export default {
  ...
  created () {
    ...
    localStorage.setItem('bbcImBoxShop', true)
  },
  ...
  methods: {
    ...
    getUserInfo () {
      this.$http({
        url: this.$http.adornUrl('/sys/shopEmployee/info'),
        method: 'GET'
      }).then(({ data }) => {
    ...
    async getConversations () {
      ...
      window.onbeforeunload = function () {
        if (that.$route.name === 'imBox') {
          localStorage.removeItem('bbcImBoxShop')
        }
      }
    ...
    openWs () {
      ...
      this.ws = new WebSocket(
        this.$http.adornWsImUrl(
          `/im/websocket/shop/${Base64.encode(this.$cookie.get('bbcAuthorization_vs'))}/${this.shopId}/${this.employeeIdMain}`
        )
      )
    ...
    // 敏感词
     getSensitiveWords(){
      this.$http({
        url: this.$http.adornUrl('/sys/webConfig/info/SENSITIVE_WORDS'),
        method: 'GET'
      }).then(({ data }) => {
        data = JSON.parse(data)
          if(data?.sensitiveWord){
            this.sensitiveWords=data.sensitiveWord.replaceAll('，','|')
          }
        })
