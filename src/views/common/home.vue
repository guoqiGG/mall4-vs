        ...
        <div class="order-all-num">
          <el-row type="flex" class="row-bg">
            ...
            <el-col :span="15" class="col-box">
              ...
              <div class="num-item-box">
                <div class="wait-evaluate-num order-num-item" @click="toStockPage(1)">
                  <div class="item-box">
                    <div class="words">{{$t("product.stockWarning")}}</div>
                    <div class="number">{{isTrueData(stockWarningNum)}}</div>
                    <!-- <div :class="['seq',nowDate?'up':'down']">
                      <span class="value">1.3%</span>
                      <span :class="nowDate?'up-icon':'down-icon'"></span>
                      <span class="compare grap">同比昨日上升{{$t("home.payCustomers")}}</span>
                  ...
                  <div class="item-img">
                    <img src="~@/assets/img/home/stockWarning.png" >
                  </div>
                </div>
              </div>
            </el-col>
              ...
              <el-col :span="15" class="col-item">
                <div class="realtime-left">
                  <div class="title">
                    ...
                    <div class="t-explain-item t-explain-first-item t-small-text t-today-data"><span>{{$t("home.dataTodayActualTotal")}}</span></div>
                    <div class="t-explain-item t-explain-second-item t-small-text t-yesterday-data"><span>{{$t("home.dataYesterdayActualTotal")}}</span></div>
                  </div>
                  <!-- 图表 -->
      ...
      <div class="order-data-right">
        <div class="data-item-box">
          <div class="data-title">
            <div class="left-title">{{ $t('notice.bulletinBoard') }}</div>
            <div class="right-title" @click="toNotify">{{ $t('notice.more') }}</div>
          </div>
          <div v-for="(item, index) in platformNotify" :key="index" class="content">{{ index + 1 + "、" + item.title }}</div>
          <div v-if="platformNotify.length <= 0" class="no-data">{{ $t('shop.noData') }}</div>
        </div>
        <!-- 消息通知 -->
        <div class="data-item-box">
          <div class="data-title">
            <div class="left-title">{{ $t('notice.messageNotification') }}</div>
            <div class="right-title" @click="toManager">{{ $t('notice.more') }}</div>
          </div>
          <div v-for="(item, index) in notifyList" :key="index" class="content notify-view" @click="notifyView(item)">
            <span style="position: relative;margin-right: 6px;" :class="item.status ? 'reads-dot': 'unread-point'">●</span>
            {{ item.message }}
          </div>
          <div v-if="notifyList.length <= 0" class="no-data">{{ $t('shop.noData') }}</div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    getSkuStockWarning () {
      this.$http({
        url: this.$http.adornUrl('/sku/stockWarningCount'),
        method: 'get'
      }).then(({ data }) => {
        this.stockWarningNum = data
      })
    },
    // 查询店铺订单各状态数量
    getOrderCount () {
    ...
    toStockPage (sts) {
      this.$router.push({
        path: '/stock-inquire?isStockWarning',
    ...
    toNotify () {
      ...
      const flag = routeList.some(item => '/' + item.url === '/notify-platform-notify/notice-list')
      if (!flag) {
        return this.$message.info(this.$i18n.t('homes.noPermissionAccessPage'))
      }
      this.$router.push({
        path: '/notify-platform-notify/notice-list'
      })
    },
    toManager () {
      const routeList = this.$store.state.common.routeList
      const flag = routeList.some(item => '/' + item.url === '/notify-notify-manager/index')
      if (!flag) {
        return this.$message.info(this.$i18n.t('homes.noPermissionAccessPage'))
      }
      this.$router.push({
        path: '/notify-notify-manager/index'
      })
    },
    getPlatformNotify () {
      this.$http({
        url: this.$http.adornUrl('/shop/notice/listPage'),
        method: 'get',
        params: this.$http.adornParams({ size: 6, current: 1 })
      }).then(({data}) => {
        this.platformNotify = data.records
      })
    },
    getNotifyList () {
      this.$http({
    ...
    notifyView (row) {
      ...
      if (row.status === 0) {
        this.$http({
          url: this.$http.adornUrl('/multishop/notifyLog/is_read?type=0'),
          method: 'put',
          data: [row.logId]
        }).then(({ data }) => {
