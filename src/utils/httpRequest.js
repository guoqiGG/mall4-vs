...
http.interceptors.request.use(
  (config) => {
    config.headers["Authorization"] = Vue.cookie.get("bbcAuthorization_vs"); // 请求头带上token
...
http.interceptors.response.use(
  (response) => {
    ...
    if (responseData.code === "A00005") {
      ...
      console.log(
        "接口地址: ",
        response.config.url.replace(process.env.VUE_APP_BASE_API, "")
      );
...
var uploadFile = function (url, file) {
  let config = {
    ...
    headers: {
      ...
      Authorization: Vue.cookie.get("bbcAuthorization_vs"), // 请求头带上token
    },
  };
